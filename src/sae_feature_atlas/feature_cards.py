...
def _top_examples_json(df: pd.DataFrame, n: int = 5) -> str:
    ...
    records = (
        df.sort_values("activation", ascending=False)
        .head(n)[existing_cols]
        .to_dict("records")
    )
...
def _drop_columns_if_present(df: pd.DataFrame, columns: list[str]) -> pd.DataFrame:
    ...
    Used to make enrichment idempotent: if cards already contain a previous
    version of enrichment columns, remove them before merging fresh values.
    """
...
def _merge_replace(
    ...
    """Merge `right` into `left`, replacing overlapping enrichment columns.

    This makes the feature-card enrichment step idempotent:
    running `cards` multiple times should update enrichment columns, not create
    `_x` / `_y` duplicates or crash with pandas MergeError.
    """
...
def build_basic_feature_cards(
    ...
    This stage only contains fields that are available immediately after feature
    statistics and top-example construction. Later analysis steps enrich this
    table with inspection, co-activation, geometry, PCA/UMAP/LDA, and bimodality
    fields.
    """
    rows: list[dict] = []
        ...
        rows.append(
            {
                ...
                "top_examples_json": _top_examples_json(group),
            }
        )
...
def build_and_save_feature_outputs(
    ...
    """Build feature stats, top examples, and initial feature cards."""
...
def _load_feature_cards_base(cfg: ExperimentConfig) -> pd.DataFrame:
    """Load feature cards, falling back to filtered features if needed.

    Normally `feature_cards.parquet` exists after the `features` step. This
    fallback makes the cards step easier to debug if the file was deleted while
    other artifacts remained.
    """
...
def _merge_bimodality(cards: pd.DataFrame, cfg: ExperimentConfig) -> pd.DataFrame:
    ...
    return _merge_replace(
        ...
        bimodal[keep],
        on="feature_id",
        columns_to_replace=[col for col in keep if col != "feature_id"],
    )
...
def _merge_decoder_neighbors(cards: pd.DataFrame, cfg: ExperimentConfig) -> pd.DataFrame:
    ...
    max_cos = (
        ...
        .rename(
            columns={
                "feature_i": "feature_id",
                "decoder_cosine": "max_decoder_cosine",
            }
        )
    )
    ...
    cards = _merge_replace(
        ...
        max_cos,
        on="feature_id",
        columns_to_replace=["max_decoder_cosine"],
    )

    neighbor_rows: list[dict] = []

    for feature_id, group in neighbors.groupby("feature_i"):
        rows = group.sort_values("rank").head(10).to_dict("records")
        neighbor_rows.append(
            {
                "feature_id": int(feature_id),
                "top_decoder_neighbors_json": _json(rows),
            }
        )
...
def _merge_coactivation(cards: pd.DataFrame, cfg: ExperimentConfig) -> pd.DataFrame:
    ...
    left = coactivation.rename(
        columns={
            "feature_i": "feature_id",
            "feature_j": "neighbor_feature_id",
        }
    )
    right = coactivation.rename(
        columns={
            "feature_j": "feature_id",
            "feature_i": "neighbor_feature_id",
        }
    )
    ...
    aggregate = (
        ...
        .agg(
            max_coactivation_jaccard=("jaccard", "max"),
            max_coactivation_pmi=("pmi", "max"),
        )
        .reset_index()
    )
    ...
    coactivation_rows: list[dict] = []

    for feature_id, group in both.groupby("feature_id"):
        rows = (
            group.sort_values(
                ["jaccard", "pmi", "coactivation_count"],
                ascending=False,
            )
            .head(10)
            .to_dict("records")
        ...
        coactivation_rows.append(
            {
                "feature_id": int(feature_id),
                "top_coactivation_neighbors_json": _json(rows),
            }
        )

    cards = _merge_replace(
        cards,
        pd.DataFrame(coactivation_rows),
        on="feature_id",
...
def _merge_decoder_pca(cards: pd.DataFrame, cfg: ExperimentConfig) -> pd.DataFrame:
    ...
    keep = [col for col in keep if col in pca.columns]
...
def _merge_decoder_umap(cards: pd.DataFrame, cfg: ExperimentConfig) -> pd.DataFrame:
    if not hasattr(cfg, "decoder_feature_umap_path"):
        return cards
    ...
    keep = [col for col in keep if col in umap_df.columns]
...
def _merge_decoder_lda(cards: pd.DataFrame, cfg: ExperimentConfig) -> pd.DataFrame:
    if not hasattr(cfg, "decoder_feature_lda_path"):
        return cards
    ...
    keep = [col for col in keep if col in lda.columns]
...
def enrich_feature_cards(cfg: ExperimentConfig) -> pd.DataFrame:
    """Enrich feature cards with all available downstream analysis artifacts.

    This function is intentionally idempotent. It is safe to run repeatedly after
    `inspection`, `bimodality`, `geometry`, `space`, or `coactivation` steps.
    """
    ...
    # Put the most useful columns near the front if they exist.
    preferred_order = [
        "feature_id",
        "primary_label",
        "manual_priority",
