...
def compute_decoder_neighbors(sae, feature_ids: list[int], top_k: int = 20, batch_size: int = 512) -> pd.DataFrame:
    ...
    for start in tqdm(range(0, len(feature_ids), batch_size), desc="Decoder neighbors"):
        ...
        sims = W_dec[batch_ids] @ W_dec.T
        for local_idx, fid in enumerate(batch_ids.tolist()):
            sims[local_idx, fid] = -1.0
        values, indices = torch.topk(sims, k=top_k, dim=-1)
        for local_idx, fid in enumerate(batch_ids.tolist()):
            for rank in range(top_k):
                rows.append({"feature_i": int(fid), "feature_j": int(indices[local_idx, rank].item()), "rank": int(rank + 1), "decoder_cosine": float(values[local_idx, rank].item())})
    return pd.DataFrame(rows)


def merge_geometry_with_coactivation(decoder_neighbors: pd.DataFrame, coactivation_pairs: pd.DataFrame) -> pd.DataFrame:
    merged = decoder_neighbors.merge(coactivation_pairs, on=["feature_i", "feature_j"], how="left")
    ...
    co_hi = float("inf") if positive_jaccard.empty else positive_jaccard.quantile(0.90)
    def quadrant(row):
